...
if __name__ == '__main__':       
    ...
    ######################################################## With Grouping With Disaggregation
    
    
    if IncludeDisAggFlag == 1:
        print("Run Algorithm with Disaggregated Cargoes")
        RunAlgo = int(input('Disaggregation: Enter 0 to print solution or 1 to run algo: '))
        #RunAlgo = 1
        
        if RunAlgo == 0:
            folder = "SolutionsGroupDis"    
            solpath = Path('%s\\%s' %(path,folder))    
            os. chdir(solpath)
            Nbr = int(input('Enter solution number: '))
            ...
            # 5. Create summary and detailed excel files of feasible solutions            
            column_names = ['SolNbr', 'DisOrderNbr', 'Feasible', 'Volume', 'NumCargoes']
            SummaryAllSolutions = pd.DataFrame(columns = column_names)    
            ...
            for l in range(len(AllSols)):                
                ...
                Vol = 0
                TotCargoes = 0
                if AllSols[SolNbr][2] == 1:
                    Vol = AllSols[SolNbr][4]
                    TotCargoes = AllSols[SolNbr][5]
                    
                data=[{'SolNbr': SolNbr, 'DisOrderNbr': AllSols[SolNbr][1], 'Feasible': AllSols[SolNbr][2], 'Volume':Vol, 'NumCargoes': TotCargoes}]        
                SummaryAllSolutions = SummaryAllSolutions.append(data,ignore_index=True)
                
                DetailsAllSolutions = DetailsAllSolutions.append(AllSols[SolNbr][3],ignore_index=True)
                DetailsAllSolutions["SolNbr"].fillna(SolNbr, inplace = True) 
            ...
            #print("Time taken to load disaggregated boxes: "+str(end - start))            
