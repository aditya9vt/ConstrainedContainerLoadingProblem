...
def LoadCargoes(Sol_P1, dfOT_P1, CDAvail_P1, set4, ContainerData):
    # Sol_P1.to_csv('Sol_P1.csv', sep=',', encoding='utf-8', index = False)            
    # print("Heuristic Called to Load Disaggregated Boxes")
    ...
    for CargoSortOption in range(3,4):
        for CDSortOption in range(2,3):
            
            ########### Sort ID_Reduced
            if CargoSortOption == 1:
                ID_Reduced = ID_Reduced.sort_values(by=['Volume'], ascending=[False])
            elif CargoSortOption == 2:
                ID_Reduced = ID_Reduced.sort_values(by=['h'], ascending=[False])
            elif CargoSortOption == 3:
                ID_Reduced = ID_Reduced.sort_values(by=['Weight'], ascending=[False])
            ...
            while IsCargoPlaced == 1:         
                ...
                while i < len(CargoRem):
                    ...
                    if IsCargoPlaced == 1:
                        ...
                        i = 10000000
            # print("Total Cargoes loaded: "+str(len(CargoLoaded_Nbr)))
            ...
            # FinalSol.to_csv('FinalSol_P1.csv', sep=',', encoding='utf-8', index = False)            
            FinalSol.drop(columns=['OrderNbr'], inplace=True)
            FinalSol.reset_index(drop=True, inplace=True)
            # FinalSol.to_csv('FinalSol_P2.csv', sep=',', encoding='utf-8', index = False)            
            
            #Extract Sno and Ordernbr from Sol_P1
            OrderNbr1 = Sol_P1[['Sno','OrderNbr']]
            OrderNbr2 = set4[['Sno','OrderNbr']]
            AllOrderNbr = OrderNbr1.append(OrderNbr2, ignore_index=True, sort=False)
            
            #Final Merge FinalSol with AllOrderNbr to populate OrderNbr for every Sno
            FinalSol = pd.merge(FinalSol, AllOrderNbr, left_on=['Sno'],right_on=['Sno'] , how='left')
            # FinalSol.to_csv('FinalSol_P3.csv', sep=',', encoding='utf-8', index = False)            
            
            if len(CargoRem) == 0:
                FeasibilityFlag = 1            
            
            #Also find list of Sno from set 4 that were not loaded
            CargoLoaded = FinalSol['Sno'].to_list()
            # dfCargoNotLoaded is automatically going to be null if final solution is feasible
            dfCargoNotLoaded = set4[~set4['Sno'].isin(CargoLoaded)]                
    ...
    return FeasibilityFlag, FinalSol, dfCargoNotLoaded, len(CargoLoaded_Nbr)
